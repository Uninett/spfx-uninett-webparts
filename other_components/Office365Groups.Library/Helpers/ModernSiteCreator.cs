...
namespace Office365Groups.Library.Helpers
{
	public class ModernSiteCreator
	{
		...
		/// <returns></returns>
		public SiteResponse CreateGroup(string displayName, string alias, bool isPublic,
																string description = "", List<string> AdditionalOwners = null)
		{
			...
			{
				foreach (var owner in AdditionalOwners.Distinct<string>())
				{
		...
		private async Task<string> CreateGroupAsync(ClientContext context, SiteRequest newSiteRequest)
		{
			...
			using (var handler = new HttpClientHandler())
			{
				...
				using (var httpClient = new HttpClient(handler))
				{
                    // helps SharePoint request throttling
                    httpClient.DefaultRequestHeaders.Add("User-Agent", AuthHelper.TRAFFIC_DECORATION);                    
					...
					var jsonSerializerSettings = new JsonSerializerSettings();
					jsonSerializerSettings.MissingMemberHandling = MissingMemberHandling.Ignore;
					var contextInformation = JsonConvert.DeserializeObject<RestRootObject>(digestTask.Result, jsonSerializerSettings);
